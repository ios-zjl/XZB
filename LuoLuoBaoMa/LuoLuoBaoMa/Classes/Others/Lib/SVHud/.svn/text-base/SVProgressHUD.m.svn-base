...
- (void)dinghao_showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
...
- (void)dinghao_dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.alpha != 1)
            return;
...
{
    [SVProgressHUD dinghao_showSuccessWithStatus:string duration:1];
}
...
+ (void)dinghao_showSuccessWithStatus:(NSString *)string duration:(NSTimeInterval)duration {
    [SVProgressHUD show];
    [SVProgressHUD dinghao_dismissWithSuccess:string afterDelay:duration];
}
...
+ (void)dinghao_dismissWithSuccess:(NSString *)successString afterDelay:(NSTimeInterval)seconds {
    [[SVProgressHUD sharedView] dinghao_dismissWithStatus:successString error:NO afterDelay:seconds];
}
...
- (void)dinghao_setStatus:(NSString *)string {
    ...
    if(string) {
        ...
        if(hudHeight > 100) {
            ...
            hudWidth+=24;
            labelRect = CGRectMake(0, 28, hudWidth, stringHeight);
        }
    }
    ...
    self.stringLabel.font = [UIFont fontWithName:@"Helvetica Neue" size:17];
    if(string)
