...
#import "Tool_FMDBGetDatabase.h" // 数据库 持久化

// ================
//#import "HistoryPasser.h"
//#import "CompanyInfoModel.h"
//#import "SysConfigModel.h"
//#import "JPNoticeModel.h"
// ================
...
//    NSString *extension = @"m4r";
   ...
   [GlobalHelper addSkipBackupAttributeToItemAtPath:path];
...
+(NSString *)GetDateStringByTimeSpan:(NSString *)timeSpan andStyle:(NSString *)style{
    NSDate *confromTimesp = [NSDate dateWithTimeIntervalSince1970:[timeSpan doubleValue]];
    NSDateFormatter* formatter = [[NSDateFormatter alloc]init];
...
+(void)DeleteAllCacheData{
    ...
    NSString *dbpath = [Tool_FMDBGetDatabase dbPath];
//    [self deleteFilesByPath:dbpath]
    ...
    [[NSUserDefaults standardUserDefaults]removeObjectForKey:USER_IsShow];// 1、显示加入企业 2、不显示加入企业
    [[NSUserDefaults standardUserDefaults]removeObjectForKey:USER_IsAdmin];// 1、普通员工 2、管理员
    
}

+(void) removeNotificationByKeyID:(NSString *)key{
    // 手动删除通知
...
// 删除文件
+(void)deleteFilesByPath:(NSString *)path{

    NSString *docsdir = [NSSearchPathForDirectoriesInDomains( NSDocumentDirectory, NSUserDomainMask, YES) lastObject];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    docsdir = [docsdir stringByAppendingPathComponent:@"xzbDB"];
     [GlobalHelper addSkipBackupAttributeToItemAtPath:docsdir];
    
    NSArray *contents = [fileManager contentsOfDirectoryAtPath:docsdir error:NULL];
    NSEnumerator *e = [contents objectEnumerator];
    NSString *filename;
    while ((filename = [e nextObject])) {
        MLog(@"%@=FileInfo_zjl=%@",[docsdir stringByAppendingPathComponent:filename],filename);
        if ([[filename pathExtension] isEqualToString:@"db"]) {
            MLog(@"删除db ： %@",filename);
         BOOL res = [fileManager removeItemAtPath:[docsdir stringByAppendingPathComponent:filename] error:NULL];
        }
    }
...
+(NSString *)getHourByMinute:(NSInteger)minute{
    ...
    NSInteger hour = minute / 60; //3小时
    NSInteger minremains = minute % 60; //余数，20分钟
    NSString *backString = nil;
    if (hour>0) {
        backString = [NSString stringWithFormat:@"%ld小时%ld分钟",hour,minremains];
    }else{
        backString = [NSString stringWithFormat:@"%ld分钟",minute];
    }
